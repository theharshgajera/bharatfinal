...
def admin_home(request):
    employee_counts = {
        ...
        'PRINT_PENDING' : EmployeeDetail.objects.filter(status='CONTRACT_PENDING').count(),
...
def edit_profile(request, pid):
    ...
    if request.method == "POST":
        ...
        employee.dob=dob
        employee.expiry_date=expiry_date
        if jdate:
            employee.joiningdate = jdate
        try:
            employee.save()
...
def verify_contract(request, employee_id):
    ...
    contract.save()
    employee_detail.status = 'PRINT_PENDING'
    employee_detail.save()
    # Send email notification
...
@login_required
def admin_print_pending(request):
    pending_documents = Document.objects.filter(verified=False)
    employees = EmployeeDetail.objects.filter(status='PRINT_PENDING')
    return render(request, 'admin_print_pending.html', {'employees': employees, 'print_pending': print_pending})

@login_required
def print_pending(request, employee_id):
    document = get_object_or_404(Document, employee_id=employee_id)
    document.verified = True
    document.save()
    employee_detail = get_object_or_404(EmployeeDetail, user=document.employee)
    ...
    employee_detail.save()
    # Send email notification
    subject = 'Card is printed'
    message = 'Your card is printed successfully'
    recipient_list = [document.employee.email]
    send_email(subject, message, recipient_list)
    return redirect('admin_print_pending')
